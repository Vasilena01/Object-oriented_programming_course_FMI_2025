...
MyString MyString::substr(int from, int length)
{
    if (from > length)
    {
        throw std::logic_error("From should be smaller number then to");
    }

    if (from > size) {
        throw std::logic_error("From should be smaller number then size");
    }
